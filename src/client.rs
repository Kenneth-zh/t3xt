...
impl Client {
    pub fn new(client_id: String) -> Result<Self> {
        ...
        info!("found cert");
        let rustls_config = create_client_config_with_cert(cert_path)?;
        let client_config = create_quinn_client_config(rustls_config);
        ...
        let recv_task = tokio::spawn(async move {
            loop {
                match recv_connection.accept_uni().await {
                    Ok(mut recvstream) => match Self::receive_message(&mut recvstream).await {
                        Ok(message) => {
